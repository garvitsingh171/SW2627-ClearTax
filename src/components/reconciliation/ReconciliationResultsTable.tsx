...
export default function ReconciliationResultsTable({
  ...
  const [pageHistory, setPageHistory] = useState([
    { rows: initialRows, nextCursor: initialNextCursor, hasMore: initialHasMore, total: initialTotal },
  ]);
  const [isLoading, setIsLoading] = useState(false);
  const [loadError, setLoadError] = useState<string | null>(null);
  const closeDrawer = useCallback(() => setSelectedRow(null), []);
  const visibleRows = useMemo(() => pageRows.filter((row) => {
  ...
  const loadPage = useCallback(async (cursor: string | null, nextFilter: Filter) => {
    setIsLoading(true);
    setLoadError(null);
    ...
    try {
      ...
      if (nextFilter !== "ALL") params.set("result", nextFilter);
      const response = await fetch(`/api/reconciliation-batches/${batchId}/results?${params}`);
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error?.message ?? "Unable to load reconciliation results.");
      return payload.data as { results: ReconciliationRowDetails[]; pagination: { nextCursor: string | null; hasMore: boolean; total: number } };
    } catch (error) {
      setLoadError(error instanceof Error ? error.message : "Unable to load reconciliation results.");
      return null;
    } finally {
      setIsLoading(false);
    }
  ...
  const changeFilter = useCallback(async (nextFilter: Filter) => {
    setFilter(nextFilter);
    setQuery("");
    const page = await loadPage(null, nextFilter);
    if (!page) return;
    const nextPage = { rows: page.results, ...page.pagination };
    setPageRows(page.results);
  ...
  const goNext = useCallback(async () => {
    ...
    const page = await loadPage(currentPage.nextCursor, filter);
    if (!page) return;
    const nextPage = { rows: page.results, ...page.pagination };
    setPageRows(page.results);
    setPageHistory((history) => [...history, nextPage]);
  }, [filter, loadPage, pageHistory]);

  const goPrevious = useCallback(() => {
    if (pageHistory.length === 1) return;
    const previousHistory = pageHistory.slice(0, -1);
    const previousPage = previousHistory[previousHistory.length - 1];
    setPageHistory(previousHistory);
    setPageRows(previousPage.rows);
  }, [pageHistory]);

  const currentPage = pageHistory[pageHistory.length - 1];
  const totalRows = currentPage.total;
